...
func (s *Service) ValidateToken(ctx context.Context, token string) bool {
	...
	if err != nil {
		...
		metricAttributes := []attribute.KeyValue{
			...
			attribute.String("event_type", "jwk"),
		}
	...
	if err != nil {
		...
		metricAttributes := []attribute.KeyValue{
			attribute.String("status", "error"),
			attribute.String("event_type", "parse"),
		}
	...
	if err := jwToken.Get(s.PrincipalIDClaims, &principalID); err != nil {
		...
		metricAttributes := []attribute.KeyValue{
			attribute.String("status", "error"),
			attribute.String("event_type", "claim"),
		}
	...
	metricAttributes := []attribute.KeyValue{
		...
		attribute.String("event_type", "validate"),
	}
